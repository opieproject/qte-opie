/****************************************************************************
** $Id: qt/src/widgets/qtabbar.cpp   2.3.10   edited 2005-01-24 $
**
...
void QTabBar::paint( QPainter * p, QTab * t, bool selected ) const
{
    ...
    int w = iw + p->fontMetrics().width( t->label ) + 4 + style().pixelMetric(QStyle::TabHMargin);
    int h = QMAX(p->fontMetrics().height() + 4, ih );
    paintLabel( p, QRect( r.left() + (r.width()-w)/2,
			  r.top() + (r.height()-h)/2,
...
#ifdef QT_KEYPAD_MODE
	if( qt_modalEditingEnabled ) {
	    style().drawFocusRect(p, br, colorGroup(), &colorGroup().highlight());
	}
#else
...
void QTabBar::show()
{
    ...
    QWidget::show();
#ifdef QT_KEYPAD_MODE
    // grab focus to make tab navigation easier
    if( qt_modalEditingEnabled ) {
	setFocus();
    }
#endif
...
void QTabBar::keyPressEvent( QKeyEvent * e )
{
...
#ifdef QT_KEYPAD_MODE
		|| (qt_modalEditingEnabled && e->key() == Key_Select)
#endif
	      ) {
	...
	if (t)
	    d->focus = t->id;
#ifdef QT_KEYPAD_MODE
	else if( qt_modalEditingEnabled ) {
	    t = lstatic->first();
	    while ( t && !t->enabled)
		t = lstatic->next();
	    if (t)
...
void QTabBar::focusOutEvent( QFocusEvent * )
{
	...
	if ( t->id == d->focus ) {
	    ...
	    int w = iw + p.fontMetrics().width( t->label ) + 4 + style().pixelMetric(QStyle::TabHMargin);
	    int h = QMAX(p.fontMetrics().height() + 4, ih );
	    p.fillRect( QRect( r.left() + ( r.width() -w ) / 2 - 1,
