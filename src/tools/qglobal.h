/****************************************************************************
** $Id: qt/src/tools/qglobal.h   2.3.10   edited 2005-01-24 $
**
...
// NOTE: 
// to allow for a two digit patch release number we have jumped from 239 to 2310
// for future reference, if we ever release a Qt 2.4.0 the version number will jump to 2400, i.e. 2.4.00
#define QT_VERSION	2310
#define QT_VERSION_STR	"2.3.10"
...
// For non-compilers that still need to parse Qt,
// like Insure++ and lint-like programs.
...
#if defined(_CC_EDG_) || defined(_CC_WAT_) || defined(_CC_USLC_)
...
#define ASSERT(x)  ((x) ? (void)0 : qFatal("ASSERT: \"%s\" in %s (%d)",#x,__FILE__,__LINE__))
#else
